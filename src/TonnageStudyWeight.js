...
const TonnageWeightAvgWeight = forwardRef((props, ref) => {
  // Load saved data from sessionStorage (if exists) when the component mounts
  const savedTableData = JSON.parse(sessionStorage.getItem('TonnageWeightAvgWeightData')) || [
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgWeight: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgWeight: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgWeight: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgWeight: '', ActualIncrease: '', PercentIncrease: '' },
  ];
  ...
  useEffect(() => {
    sessionStorage.setItem('TonnageWeightAvgWeightData', JSON.stringify(tableData));
  }, [tableData]);
 ...
 const handlePaste = (index, field, event) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    calculateGraphData() {
      ...
      let previousAvgWeight = null; // Track the previous row's average WeightAvgWeight
      ...
      const data = sortedData.map((row, index) => {
        ...
        const avgWeightAvgWeight = (Sample1 + Sample2 + Sample3) / 3;
        ...
        if (previousAvgWeight !== null) {
          ...
          actualIncrease = avgWeightAvgWeight - previousAvgWeight;
          percentIncrease = (actualIncrease / previousAvgWeight) * 100;
        }
  
        // Update row data with calculated values
        row.AvgWeight = avgWeightAvgWeight.toFixed(2);
        row.ActualIncrease = actualIncrease.toFixed(2);
        row.PercentIncrease = percentIncrease.toFixed(2);
  
        previousAvgWeight = avgWeightAvgWeight; // Update the previous average WeightAvgWeight for the next iteration
  
        return {
          Tonnage: parseFloat(row.Tonnage),
          AvgWeight: avgWeightAvgWeight,
        };
      });
  ...
  const handleKeyDown = (event, rowIndex, field) => {
    ...
    const fieldMapping = {
      'Tonnage': 0,'Sample1': 1,'Sample2': 2,'Sample3': 3};
    ...
    <div className="page-container">
      ...
      <div className="table-container">
        ...
        <table className="data-table">
          <thead>
            <tr>
              <th>Tonnage</th><th>Sample 1</th><th>Sample 2</th><th>Sample 3</th>
              <th>Avg Weight</th><th>Actual Increase</th><th>% Increase</th>
            </tr>
          </thead>
          <tbody>
            {tableData.map((row, rowIndex) => (
              <tr key={rowIndex}>
                ...
                <td className="grey-background">{row.AvgWeight}</td>
...
export default TonnageWeightAvgWeight;
