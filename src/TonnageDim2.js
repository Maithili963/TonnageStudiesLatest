...
const TonnageDim2 = forwardRef((props, ref) => {
  // Load saved data from sessionStorage (if exists) when the component mounts
  const savedTableData = JSON.parse(sessionStorage.getItem('TonnageDim2Data')) || [
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgDim2: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgDim2: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgDim2: '', ActualIncrease: '', PercentIncrease: '' },
    { Tonnage: '', Sample1: '', Sample2: '', Sample3: '', AvgDim2: '', ActualIncrease: '', PercentIncrease: '' },
  ];
  ...
  useEffect(() => {
    sessionStorage.setItem('TonnageDim2Data', JSON.stringify(tableData));
  }, [tableData]);
...
const handlePaste = (index, field, event) => {
  ...
  useImperativeHandle(ref, () => ({ 
    ...
    calculateGraphData() {
      ...
      let previousAvgDim2 = null; // Track the previous row's average Dim1
      ...
      const data = sortedData.map((row, index) => {
        ...
        const avgDim2 = (Sample1 + Sample2 + Sample3) / 3;
  
        let actualIncrease = 0;
        let percentIncrease = 0;
  
        if (previousAvgDim2 !== null) {
          // Apply the formulas after sorting
          actualIncrease = avgDim2 - previousAvgDim2;
          percentIncrease = (actualIncrease / previousAvgDim2) * 100;
        }
  
        // Update row data with calculated values
        row.AvgDim2 = avgDim2.toFixed(2);
        row.ActualIncrease = actualIncrease.toFixed(2);
        row.PercentIncrease = percentIncrease.toFixed(2);
  
        previousAvgDim2 = avgDim2; // Update the previous average Dim1 for the next iteration
  ...
  const handleKeyDown = (event, rowIndex, field) => {
    ...
    // Handle "Enter" key to move to the next row's "Tonnage" column
    if (key === 'Enter') {
          ...
          <tbody>
            {tableData.map((row, rowIndex) => (
              <tr key={rowIndex}>
                ...
                <td className="grey-background">{row.AvgDim2}</td>
